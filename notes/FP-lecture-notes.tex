...
\red{
\begin{itemize}
\item Describe substitutions
\item Textually describe unification algorithm
\item Define rest of the context stuff
\item In W definition : need to comment the need for fresh type variables $\beta$s
\item In W definition : need to comment that when sub-call fail, then also the calling computation fails
\item write Intro
\end{itemize}
}
...
\red{replace simple-type with mono-type}
...
\red{specifiy the convetions regarding $\sigma$ for poly-types (wel more precisely any-type) and  $\tau$ for mono-types}
...
\red{TODO: formulate more precisely/correctly...}
\end{itemize}
